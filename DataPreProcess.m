...
for subject=1:totalsubject
    ...
    if (strcmp(dataset,'Bench'))
        str =  'D:\SSVEP\benchmarkdata\';
        % str = strcat(str,'S',num2str(subject),'.mat\','S',num2str(subject));
        str = strcat(str,'S',num2str(subject),'.mat\');
        subject_data = load(str);
        data=subject_data.data;
    end
    %% 使用Beta数据集
    if strcmp(dataset,'BETA')
        str = 'D:\SSVEP\betadata\';
        str = strcat(str,'S',num2str(subject));
        subject_data = load(str);
		data=subject_data.data.EEG;	
	end
